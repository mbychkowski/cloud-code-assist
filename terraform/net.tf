...
resource "google_compute_router" "router" {
  for_each   = toset(values(module.vpc.subnets_regions))
  project    = local.project.id
  name       = "nat-router-${var.customer_id}-${each.value}"
...
module "cloud-nat" {
  ...
  project_id                         = local.project.id
  region                             = each.value
  router                             = "nat-router-${var.customer_id}-${each.value}"
  name                               = "nat-${var.customer_id}-${each.value}"
